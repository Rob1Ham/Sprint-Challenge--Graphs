...
#Prior to travesing the entire map, we need to understand which rooms are connected to each other.
...
def room_recursive(starting_room,room_graph,room_paths=None,visited=None):
    ...
    DEPRECATED VARIABLE
    origin = the direction traveled from to get to that room, None on the first call since we are placed in room
    '''
    
    #If no room has been visited yet, initialize an empty list
    if visited is None:
        visited = []
    #if a room's directions is not defined yet, create a set
